...
window.filterProducts = function (category) {
    // 1. Update Active Class (Visual Feedback)
    document.querySelectorAll('.category-list li').forEach(li => li.classList.remove('active'));
    
    // Find the button that matches the category text
    const activeBtn = Array.from(document.querySelectorAll('.category-list li')).find(li => {
        const text = li.textContent.toLowerCase().trim();
        const cat = category.toLowerCase().trim();
        return text.includes(cat === 'all' ? 'all' : cat);
    });
    ...
    if (category !== 'all') {
        // specific comparison to avoid case-sensitivity errors
        filteredList = products.filter(p => (p.category || '').toLowerCase() === category.toLowerCase());
    }

    renderShop(filteredList);

    // 3. Close mobile menu ONLY if it is currently open
    // (Prevents the menu from popping open when clicking filters on Desktop)
...
function createCardHTML(product) {
    let favorites = JSON.parse(localStorage.getItem('RDTradingFavs')) || [];
    const isFav = favorites.some(fav => fav.id === product.id) ? 'active' : '';

    // --- 1. Price Logic (Existing) ---
    const sqFtCategories = ['whiteboard', 'greenboard', 'noticeboard', 'partition'];
    const isSqFt = sqFtCategories.includes((product.category || '').toLowerCase().trim());
    
    let priceDisplay;
    if (isSqFt) {
        priceDisplay = `Rs.${product.price.toLocaleString()}.00 <span style="font-size:0.8em; color:#666;">(Per sq.ft)</span>`;
    } else {
        priceDisplay = `Rs.${product.price.toLocaleString()}.00`;
    }

    // --- 2. Stock Logic (New) ---
    // Check your sheet's column name. Assuming 'stock' column with values 'yes'/'no' or '1'/'0'
    // If column is missing, we assume it's in stock by default
    const inStock = (product.stock === undefined) || 
                    (String(product.stock).toLowerCase() === 'yes') || 
    ...
    // Disable enquire button if out of stock (Optional)
    const btnState = inStock ? '' : 'style="opacity:0.6; pointer-events:none;"';
    const btnText  = inStock ? 'ENQUIRE NOW <i class="fab fa-whatsapp"></i>' : 'SOLD OUT';
    ...
    return `
        <div class="product-card">
            ...
            <!-- Stock Badge Positioned over Image -->
            ${stockLabel}
            ...
            <a href="https://wa.me/94740611225?text=Hi RD Trading, I am interested in ${product.name}" class="btn-options" target="_blank" ${btnState}>
                ${btnText}
            </a>
        </div>
    `;
...
window.toggleLike = function (id, btnElement) {
    ...
    localStorage.setItem('RDTradingFavs', JSON.stringify(favorites));
    updateFavCount();
